...
create table if not exists public.trades (
  ...
  status text not null, -- 'filled', 'cancelled' for spot; 'active', 'settled' for contract
  amount double precision not null, -- For spot, it's base asset amount. For contract, it's quote asset investment.
  total double precision, -- For spot trades, total value in quote asset.
...
create table if not exists public.announcements (
    ...
    constraint unique_type_for_singletons check (type not in ('carousel') or user_id is null)
);
...
comment on table public.market_kline_data is 'Stores OHLC (candlestick) data for market charts. Consider partitioning for very large datasets.';
...
values (1, '{"trade_fee": 0.001, "kline_refresh_interval_sec": 60, "check_in_reward_base": 0.5}')
...
insert into public.supported_assets (asset, is_active, asset_type) values
('USDT', true, 'crypto'),
('BTC', true, 'crypto'),
('ETH', true, 'crypto'),
('USD', true, 'fiat'),
...
begin
    ...
    for v_asset_record in select asset from public.supported_assets where is_active = true loop
...
begin
    if p_is_debit_frozen then
        ...
        select available_balance into v_available from public.balances where user_id = p_user_id and asset = p_asset for update;
        if v_available < p_amount then
            raise exception 'Insufficient available balance to freeze for user %, asset %', p_user_id, p_asset;
        end if;
...
begin
  return (select sum(available_balance + frozen_balance) from public.balances where asset_type = 'crypto' limit 1);
end;
$$ language plpgsql;
...
begin
    -- Only distribute commissions for trades with a valid quote asset
    if new.quote_asset is not null then
        ...
        for v_level in 1..3 loop
            ...
            select rate into v_rate from public.commission_rates where level = v_level;
            ...
            values (v_inviter_id, 'team', v_commission_amount, new.quote_asset, new.id::text, v_source_user.username, v_level, 'Level ' || v_level || ' commission from trade ' || new.id);
...
begin
    ...
    if v_last_check_in_date = current_date then
        return query select false, 'You have already checked in today.', 0.0;
        return;
    end if;

    if v_last_check_in_date = current_date - interval '1 day' then
...
begin
    ...
    loop
        -- Simulate fetching the current price for settlement
        ...
        set status = 'settled', settlement_price = settlement_price, outcome = outcome, profit = profit
        where id = trade_record.id;

        if outcome = 'win' then
            perform public.adjust_balance(trade_record.user_id, quote_asset, trade_record.amount + profit, false, true);
        else
            perform public.adjust_balance(trade_record.user_id, quote_asset, 0, false, true);
        end if;
...
begin
    ...
    values ('settle_due_records', 'started', now()) returning id into log_id;

    begin
        result_text := public.settle_due_records();
        update public.cron_job_logs
        set run_status = 'success', details = result_text, end_time = now()
        where id = log_id;
    exception when others then
        ...
        update public.cron_job_logs
        set run_status = 'failed', details = 'SQLSTATE: ' || v_sqlstate || ' | ' || v_message || ' | CONTEXT: ' || v_context, end_time = now()
        where id = log_id;
    end;
end;
$$ language plpgsql;
...
create policy "Admins can do anything to profiles" on public.profiles for all using ((select is_admin from public.profiles where id = auth.uid()));
...
create policy "Allow read access to non-personal announcements" on public.announcements for select using (user_id is null);
drop policy if exists "Users can view their own messages" on public.announcements;
create policy "Users can view their own messages" on public.announcements for select using (auth.uid() = user_id);

-- Admin Policies for management tables
drop policy if exists "Admins can manage system_settings" on public.system_settings;
create policy "Admins can manage system_settings" on public.system_settings for all using ((select is_admin from public.profiles where id = auth.uid()));
drop policy if exists "Admins can manage daily_tasks" on public.daily_tasks;
create policy "Admins can manage daily_tasks" on public.daily_tasks for all using ((select is_admin from public.profiles where id = auth.uid()));
...
create policy "Admins can manage commission_rates" on public.commission_rates for all using ((select is_admin from public.profiles where id = auth.uid()));
...
select cron.schedule(
    ...
    '0 1 * * *', -- Run once daily at 1 AM UTC
    $$
    delete from public.action_logs where created_at < now() - interval '30 days';
    delete from public.cron_job_logs where start_time < now() - interval '30 days';
    $$
);
