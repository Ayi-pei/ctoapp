...
interface EnhancedSystemSettingsContextType {
  ...
  updateDepositAddress: (
    asset: keyof EnhancedSystemSettings["depositAddresses"],
    value: string
  ) => void;
  updateSetting: <
  ...
  >(
    key: K,
    value: EnhancedSystemSettings[K]
  ) => void;
  updatePairSettings: (
    pair: string,
    newSettings: Partial<EnhancedTradingPairSettings>
  ) => void;
  updateRiskControlSettings: (
    newSettings: Partial<RiskControlSettings>
  ) => void;
  updateSystemMonitoringSettings: (
    newSettings: Partial<SystemMonitoringSettings>
  ) => void;

  // 增强的干预管理功能
  addMarketIntervention: (
    intervention?: Partial<EnhancedMarketIntervention>
  ) => string;
  ...
  resolveInterventionConflicts: (
    interventions: EnhancedMarketIntervention[]
  ) => EnhancedMarketIntervention | null;
...
const defaultMarketSettings: { [key: string]: EnhancedTradingPairSettings } =
...
const isRecurringActive = (
  currentDate: Date,
  recurring?: EnhancedMarketIntervention["recurring"]
): boolean => {
...
const EnhancedSystemSettingsContext = createContext<
  EnhancedSystemSettingsContextType | undefined
>(undefined);

export function EnhancedSystemSettingsProvider({
  children,
}: {
  children: ReactNode;
}) {
  const { addLog } = useSimpleEnhancedLogs();
  const [systemSettings, setSystemSettings] = useState<EnhancedSystemSettings>(
  ...
  useEffect(() => {
    try {
      ...
      console.error(
        "Failed to load enhanced system settings from localStorage",
        error
      );
    }
    setIsLoaded(true);
  ...
  useEffect(() => {
    if (isLoaded) {
      try {
        ...
        console.error(
          "Failed to save enhanced system settings to localStorage",
          error
        );
      }
    }
  ...
  const updatePairSettings = useCallback(
    (pair: string, newSettings: Partial<EnhancedTradingPairSettings>) => {
      setSystemSettings((prev) => ({
        ...prev,
        marketSettings: {
          ...prev.marketSettings,
  ...
  const addMarketIntervention = useCallback(
    (intervention?: Partial<EnhancedMarketIntervention>): string => {
      ...
      // 临时注释日志记录，避免循环依赖
      console.log(
        `Created market intervention for ${newIntervention.tradingPair}`
      );
  ...
  const removeMarketIntervention = useCallback(
    (id: string) => {
  ...
  const updateMarketIntervention = useCallback(
    (id: string, updates: Partial<EnhancedMarketIntervention>) => {
      const intervention = systemSettings.marketInterventions.find(
        (i) => i.id === id
      );
      if (!intervention) return;
  ...
  const deactivateIntervention = useCallback(
    (id: string) => {
  ...
  // 增强的时间逻辑验证
  const isInterventionActiveNow = useCallback(
    (intervention: EnhancedMarketIntervention): boolean => {
      if (!intervention.isActive) return false;

      const now = new Date();

      // 检查日期范围
      if (!isDateInRange(now, intervention.startDate, intervention.endDate))
        return false;

      // 检查时间范围
      if (!isTimeInRange(now, intervention.startTime, intervention.endTime))
        return false;
  ...
  const getActiveInterventions = useCallback(
    (tradingPair?: string): EnhancedMarketIntervention[] => {
      let activeInterventions = systemSettings.marketInterventions.filter(
        isInterventionActiveNow
      );

      if (tradingPair) {
        activeInterventions = activeInterventions.filter(
          (i) => i.tradingPair === tradingPair
        );
      }
  ...
  const resolveInterventionConflicts = useCallback(
    (
      interventions: EnhancedMarketIntervention[]
    ): EnhancedMarketIntervention | null => {
  ...
  // 验证功能
  const validateInterventionSettings = useCallback(
    (
      intervention: Partial<EnhancedMarketIntervention>
    ): { valid: boolean; errors: string[] } => {
      const errors: string[] = [];
      ...
      if (
        intervention.minPrice &&
        intervention.maxPrice &&
        intervention.minPrice >= intervention.maxPrice
      ) {
      ...
      if (
        ...
        (intervention.priority < 1 || intervention.priority > 10)
      ) {
        errors.push("Priority must be between 1 and 10");
      }

      if (intervention.startTime && intervention.endTime) {
        const [startHour, startMin] = intervention.startTime
          .split(":")
          .map(Number);
      ...
      if (
        intervention.maxPriceDeviation &&
        intervention.maxPriceDeviation >
          systemSettings.riskControl.maxPriceDeviationGlobal
      ) {
        errors.push(
          `Price deviation exceeds global limit of ${
            systemSettings.riskControl.maxPriceDeviationGlobal * 100
          }%`
        );
      }
  ...
  const importSettings = useCallback(
    (settingsJson: string): { success: boolean; error?: string } => {
      try {
  ...
  const restoreFromBackup = useCallback(
    (backupData: string): { success: boolean; error?: string } => {
      try {
...
export function useEnhancedSystemSettings() {
  ...
  if (context === undefined) {
    throw new Error(
      "useEnhancedSystemSettings must be used within an EnhancedSystemSettingsProvider"
    );
  }
  return context;
}
