...
export function TasksProvider({ children }: { children: ReactNode }) {
  ...
  const fetchTaskStates = useCallback(async () => {
    ...
    try {
      ...
      // 获取其他数据来计算任务状态
      const [initialInvestmentRes, snowballRes, marketPredictionRes] =
        await Promise.all([
          supabase
            .from("investments")
            .select("id")
        ...
        if (claimedSet.has(task.key)) {
          ...
          switch (task.key) {
            case "initial_investment":
              ...
              status =
                investments && investments.length > 0 ? "ELIGIBLE" : "LOCKED";
              progress.current = investments && investments.length > 0 ? 1 : 0;
              break;
            case "snowball":
              const { data: deposits } = snowballRes;
              const totalDeposit =
                deposits?.reduce((acc, t) => acc + t.amount, 0) || 0;
              progress = {
                current: Math.min(totalDeposit, 1000),
                target: 1000,
              };
  ...
  const triggerTaskCompletion = async (triggerType: TaskTriggerType) => {
    ...
    const { error } = await supabase.rpc("handle_task_completion", {
    ...
    if (error) {
      ...
      console.error(
        `Failed to trigger task completion for ${triggerType}:`,
        error.message
      );
    } else {
