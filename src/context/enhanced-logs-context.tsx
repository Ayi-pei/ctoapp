...
interface EnhancedLogsContextType {
  ...
  addInterventionLog: (
    log: Omit<MarketInterventionLog, "id" | "timestamp">
  ) => void;
  trackPerformance: (
    interventionId: string,
...
const EnhancedLogsContext = createContext<EnhancedLogsContextType | undefined>(
  undefined
);
...
export function EnhancedLogsProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      ...
      // 加载干预日志
      const storedInterventionLogs = localStorage.getItem(
        INTERVENTION_LOGS_STORAGE_KEY
      );
    ...
    if (isLoaded) {
      try {
        ...
        localStorage.setItem(
          INTERVENTION_LOGS_STORAGE_KEY,
          JSON.stringify(interventionLogs)
        );
      } catch (error) {
  ...
  const addLog = useCallback(
    ...
    [adminUser, getClientInfo, addAuditLog]
  );

  // 市场干预日志记录
  const addInterventionLog = useCallback(
    (log: Omit<MarketInterventionLog, "id" | "timestamp">) => {
      ...
      setInterventionLogs((prev) =>
        [newInterventionLog, ...prev].slice(0, 1000)
      ); // 保留最近1000条干预记录
  ...
  const trackPerformance = useCallback(
    (interventionId: string, executionTime: number, priceImpact?: number) => {
      setPerformanceMetrics((prev) => {
        ...
        if (priceImpact !== undefined) {
          const history =
            newMetrics.priceDeviationHistory.get(interventionId) || [];
          history.push(priceImpact);
          newMetrics.priceDeviationHistory.set(
            interventionId,
            history.slice(-100)
          ); // 保留最近100次记录
        }
        ...
        const currentLoad = {
          ...
          activeInterventions: interventionLogs.filter(
            (log) => Date.now() - log.timestamp < 24 * 60 * 60 * 1000 // 24小时内的干预
          ).length,
        };
        newMetrics.systemLoad = [...newMetrics.systemLoad, currentLoad].slice(
          -1000
        ); // 保留最近1000个负载记录
  ...
  const getInterventionLogsByPair = useCallback(
    (tradingPair: string) => {
...
export function useEnhancedLogs() {
  ...
  if (context === undefined) {
    throw new Error(
      "useEnhancedLogs must be used within an EnhancedLogsProvider"
    );
  }
  return context;
}
