...
export function SwapProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "swap_orders" },
        () => {
          fetchOrders();
        }
      )
  ...
  const confirmCompletion = async (orderId: string) => {
    ...
    if (!order || order.status !== "pending_confirmation" || !isSupabaseEnabled)
      return;
    ...
    if (error) {
      ...
      try {
        ...
        console.error(
          "Critical error: Failed to revert transfers:",
          revertError
        );
        toast({
          variant: "destructive",
