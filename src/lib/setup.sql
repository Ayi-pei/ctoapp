...
CREATE TABLE public.users (
    ...
    inviter_id UUID REFERENCES public.users(id) ON DELETE
    SET NULL,
...
CREATE OR REPLACE FUNCTION public.generate_invitation_code() RETURNS TRIGGER AS $$ BEGIN NEW.invitation_code := upper(
        substr(
...
CREATE TRIGGER on_new_user_before_insert BEFORE
INSERT ON public.users FOR EACH ROW EXECUTE FUNCTION public.generate_invitation_code();
...
admin_codes TEXT [] := ARRAY ['147258','admin8888','admina6666','adminayi'];
is_admin_code BOOLEAN := p_invitation_code = ANY(admin_codes);
BEGIN -- Check if username exists in public.users
IF EXISTS (
    SELECT 1
    FROM public.users
    WHERE username = p_username
) THEN RETURN json_build_object('status', 'error', 'message', '用户名已存在');
END IF;
-- Handle invitation code logic
IF is_admin_code THEN v_inviter_id := NULL;
ELSE
SELECT id INTO v_inviter_id
FROM public.users
WHERE invitation_code = p_invitation_code;
IF v_inviter_id IS NULL THEN RETURN json_build_object('status', 'error', 'message', '无效的邀请码');
END IF;
END IF;
...
DECLARE commission_rates NUMERIC [] := ARRAY [0.08, 0.05, 0.02];
...
EXIT
WHEN current_inviter_id IS NULL;
...
CREATE OR REPLACE FUNCTION public.after_contract_trade() RETURNS TRIGGER AS $$ BEGIN IF NEW.status = 'settled' THEN PERFORM public.distribute_commissions(NEW.user_id, NEW.amount);
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
AFTER
INSERT ON public.contract_trades FOR EACH ROW EXECUTE FUNCTION public.after_contract_trade();
-- Function for admin to get all users.
CREATE OR REPLACE FUNCTION public.admin_get_all_users() RETURNS SETOF public.users LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN IF (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
    ) THEN RETURN QUERY
...
ELSE RAISE EXCEPTION 'Insufficient privileges';
END IF;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.get_user_downline(p_user_id UUID) RETURNS TABLE(
        id UUID,
        username TEXT,
        level INT,
        created_at TIMESTAMPTZ
    ) AS $$ BEGIN RETURN QUERY WITH RECURSIVE downline_cte AS (
        SELECT u.id,
            u.username,
            u.created_at,
            1 AS level
...
CREATE POLICY "Admin full access" ON public.transactions FOR ALL USING (
    (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
...
CREATE POLICY "Admin full access" ON public.contract_trades FOR ALL USING (
    (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
...
CREATE POLICY "Admin full access" ON public.spot_trades FOR ALL USING (
    (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
...
CREATE POLICY "Admin full access" ON public.commission_logs FOR ALL USING (
    (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
...
CREATE POLICY "Admin full access" ON public.investments FOR ALL USING (
    (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
...
CREATE POLICY "Admin full access" ON public.withdrawal_addresses FOR ALL USING (
    (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
...
CREATE POLICY "Admin full access" ON public.admin_requests FOR ALL USING (
    (
        SELECT is_admin
        FROM public.users
        WHERE id = auth.uid()
...
CREATE POLICY "Users can view their own commission logs" ON public.commission_logs FOR
SELECT USING (auth.uid() = upline_user_id);
----------------------------------------------------------------
...
test_user_id UUID;
BEGIN -- 清理已存在的邮箱，避免唯一约束冲突
DELETE FROM auth.users
WHERE email IN ('admin@noemail.app', 'testuser@noemail.app');
