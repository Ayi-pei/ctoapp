    ...
    <>
      ...
      <div className="py-6 space-y-4 text-sm">
        <div className="flex justify-between">
          ...
          <span className={cn("font-semibold", orderTypeColor)}>
            {asset} {orderTypeText}
          </span>
        </div>
        <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <span className="font-semibold text-green-500">
            {(parseFloat(amount) * currentProfitRate).toFixed(2)} {quoteAsset} (
            {(currentProfitRate * 100).toFixed(0)}%)
          </span>
        </div>
      </div>
  ...
  const renderOrderCreationView = () => (
    <>
          ...
          <div className="grid grid-cols-4 gap-2">
            {periods.map((p) => (
              <Button
                key={p.value}
                variant={
                  selectedPeriod.value === p.value ? "default" : "secondary"
                }
      ...
      <SheetFooter className="flex-col space-y-4">
        ...
        <Button onClick={handleInitialConfirm} size="lg" className="w-full">
          确定
        </Button>
      </SheetFooter>
    </>
