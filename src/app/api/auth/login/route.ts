...
export async function POST(request: Request) {
    try {
        ...
        // 2. Regular user login using DB + hashed password
        ...
        if (!userProfile.password_hash) {
          return NextResponse.json({ success: false, error: 'Account not configured' }, { status: 400 });
        }
        ...
        const safeUser: User = {
          ...
          email: userProfile.email ?? `${username}@noemail.app`,
