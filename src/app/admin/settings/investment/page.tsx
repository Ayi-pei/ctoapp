    ...
    <div className="relative pt-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id={`product-rate-${product.id}`}
            type="number"
            min={0}
            step="0.01"
            value={(product.dailyRate || 0) * 100}
            onChange={(e) =>
              updateProduct(product.id, {
                dailyRate: parseFloat(e.target.value) / 100 || 0,
              })
            }
            placeholder="例如: 3 (代表3%)"
          />
        </div>
        <div className="space-y-2">
          <Label htmlFor={`product-period-${product.id}`}>周期 (天)</Label>
          <Input
            id={`product-period-${product.id}`}
            type="number"
            min={1}
            step="1"
        ...
        <div className="space-y-2">
          ...
          <Input
            ...
            onChange={(e) =>
              updateProduct(product.id, {
                maxPurchase: parseInt(e.target.value) || 0,
              })
            }
            placeholder="例如: 1"
          />
        ...
        <div className="space-y-2">
          ...
          <Input
            id={`staking-asset-${product.id}`}
            value={product.stakingAsset || ""}
            onChange={(e) =>
              updateProduct(product.id, { stakingAsset: e.target.value })
            }
            placeholder="例如: USDT"
          />
        </div>
        <div className="space-y-2">
          <Label htmlFor={`staking-amount-${product.id}`}>
            质押数量 (选填)
          </Label>
          <Input
            ...
            onChange={(e) => {
              ...
              updateProduct(product.id, {
                stakingAmount: val ? parseFloat(val) : undefined,
              });
            }}
            ...
            {product.imgSrc && (
              <div className="flex items-center gap-2">
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => updateProduct(product.id, { imgSrc: "" })}
                  className="h-8 w-8 text-muted-foreground hover:text-destructive hover:bg-destructive/10"
                >
                  <Trash2 className="h-4 w-4" />
                </Button>
...
const FinanceProductEditor = ({
  ...
  const handleTierChange = (
    ...
    const newTiers = currentTiers.map((tier, index) => {
      ...
      const updatedValue =
        field === "rate" ? numValue / 100 : Math.max(0, numValue);
    ...
    <div className="relative pt-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6">
        ...
        <div className="space-y-2">
          ...
          <Input
            id={`fin-start-time-${product.id}`}
            type="time"
            value={product.activeStartTime}
            onChange={(e) =>
              updateProduct(product.id, { activeStartTime: e.target.value })
            }
          />
        </div>
        <div className="space-y-2">
          ...
          <Input
            id={`fin-end-time-${product.id}`}
            type="time"
            value={product.activeEndTime}
            onChange={(e) =>
              updateProduct(product.id, { activeEndTime: e.target.value })
            }
          />
        </div>
            ...
            {(product.hourlyTiers || []).map((tier, index) => (
              ...
              >
                <Input
                  ...
                  value={tier.hours}
                  onChange={(e) =>
                    handleTierChange(index, "hours", e.target.value)
                  }
                  placeholder="小时"
                  className="w-1/3"
                />
                ...
                <Input
                  ...
                  value={tier.rate * 100}
                  onChange={(e) =>
                    handleTierChange(index, "rate", e.target.value)
                  }
                  placeholder="利率"
                  className="w-1/3"
                />
...
export default function AdminInvestmentSettingsPage() {
  const { investmentProducts, addProduct, removeProduct, updateProduct } =
    useInvestmentSettings();
  ...
  const stakingProducts = investmentProducts.filter(
    (p) => p.category === "staking"
  );
  const financeProducts = investmentProducts.filter(
