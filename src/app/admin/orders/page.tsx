...
export default function AdminOrdersPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const formatted = combinedOrders
      .map((o: Record<string, any>) => {
        ...
        return {
          ...baseFormatted,
          statusText: getOrderStatusText(baseFormatted),
        };
      })
      .sort(
        (a, b) =>
  ...
  const filteredOrders = useMemo(() => {
    return allOrders.filter((order) => {
      const typeMatch =
        typeFilter === "all" || order.orderTypeText === typeFilter;
      const statusMatch =
        statusFilter === "all" || order.statusText === statusFilter;
      const dateMatch =
        !dateFilter ||
  ...
  const getOrderAmount = (order: FormattedOrder): string => {
    ...
    if (order.orderTypeText === "investment")
      return (order as Investment).amount.toFixed(2);
    if (order.orderTypeText === "swap")
      return `${(order as SwapOrder).from_amount.toFixed(4)} -> ${(
        order as SwapOrder
      ).to_amount.toFixed(4)}`;
    return "N/A";
  };
  ...
  const getStatusBadge = (order: FormattedOrder) => {
    ...
    if (order.orderTypeText === "contract") {
      ...
      if (contract.outcome === "win")
        return (
          <Badge variant="outline" className="text-green-500">
            {text} (+{(contract.profit || 0).toFixed(2)})
          </Badge>
        );
      if (contract.outcome === "loss")
        return (
    ...
    if (order.orderTypeText === "investment") {
      ...
      if (investment.status === "settled")
        return (
          <Badge variant="outline" className="text-green-500">
            {text} (+{(investment.profit || 0).toFixed(2)})
          </Badge>
        );
    }
    if (order.orderTypeText === "swap") {
  ...
  const getPairOrProduct = (order: FormattedOrder) => {
    if ("trading_pair" in order && order.trading_pair)
      return order.trading_pair;
    if ("product_name" in order && order.product_name)
      return order.product_name;
    if ("from_asset" in order && "to_asset" in order)
      return `${order.from_asset}/${order.to_asset}`;
    return "N/A";
  };
  ...
  const getPriceInfo = (order: FormattedOrder) => {
    ...
    if (order.orderTypeText === "contract") {
      ...
      if (
        contractTrade.status === "settled" &&
        contractTrade.settlement_price
      ) {
        return (
                      ...
                      <TableCell>
                        <span
                          className={cn(
                            ...
                            order.orderTypeText === "spot" &&
                              "bg-blue-500/20 text-blue-500",
                            order.orderTypeText === "contract" &&
                              "bg-purple-500/20 text-purple-500",
                            order.orderTypeText === "investment" &&
                              "bg-yellow-500/20 text-yellow-500",
                            order.orderTypeText === "swap" &&
