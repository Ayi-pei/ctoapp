    ...
    <Card className="p-4 space-y-4 relative">
      <div className="flex items-center justify-between">
        ...
        <Badge
          ...
          className={cn(activity.status === "published" && "bg-green-500/80")}
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Textarea
              id={`activity-reward-${activity.id}`}
              value={activity.rewardRule}
              onChange={(e) =>
                updateActivity(activity.id, { rewardRule: e.target.value })
              }
            />
          </div>
        </div>
        <div className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor={`activity-claim-${activity.id}`}>
              奖励领取方式
            </Label>
            <Input
              id={`activity-claim-${activity.id}`}
              value={activity.howToClaim}
              onChange={(e) =>
                updateActivity(activity.id, { howToClaim: e.target.value })
              }
            />
          </div>
          <div className="space-y-2">
            ...
            <Input
              id={`activity-expiry-${activity.id}`}
              type="date"
              value={activity.expiresAt.split("T")[0]}
      ...
      <div className="p-4 md:p-8 space-y-6 bg-card/80 backdrop-blur-sm">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              设置前端用户“限时活动”板块内容。可编辑活动标题、内容、奖励规则、有效期等。
            </CardDescription>
          </CardHeader>
