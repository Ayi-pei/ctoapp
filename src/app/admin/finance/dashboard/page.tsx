...
export default function AdminFinanceDashboardPage() {
  ...
  const financialStats = useMemo(() => {
    ...
    const calculateTotal = (reqs: AnyRequest[]) =>
      reqs.reduce((sum, r) => sum + ((r as any).amount || 0), 0);

    const dailyDeposits = calculateTotal(
      deposits.filter((r) => isToday(parseISO(r.created_at)))
    );
    const monthlyDeposits = calculateTotal(
      deposits.filter((r) => isThisMonth(parseISO(r.created_at)))
    );
    const totalDeposits = calculateTotal(deposits);

    const dailyWithdrawals = calculateTotal(
      withdrawals.filter((r) => isToday(parseISO(r.created_at)))
    );
    const monthlyWithdrawals = calculateTotal(
      withdrawals.filter((r) => isThisMonth(parseISO(r.created_at)))
    );
  ...
  useEffect(() => {
    const calculateUserStats = async () => {
      ...
      const onlineUsers = allUsers.filter(
        (u) => u.last_login_at && u.last_login_at > fiveMinAgo
      ).length;
      const registeredToday = allUsers.filter((u) =>
        isToday(parseISO(u.created_at))
      ).length;
      const offline48h = allUsers.filter(
        (u) => !u.last_login_at || u.last_login_at < fortyEightHoursAgo
      ).length;
  ...
  const chartData = [
    {
      name: "总计",
      充值: financialStats.totalDeposits,
      提现: financialStats.totalWithdrawals,
    },
    {
      name: "本月",
      充值: financialStats.monthlyDeposits,
      提现: financialStats.monthlyWithdrawals,
    },
    {
      name: "今日",
      充值: financialStats.dailyDeposits,
      提现: financialStats.dailyWithdrawals,
    },
  ];
      ...
      <div className="p-4 md:p-8 space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div className="grid gap-4 grid-cols-2 grid-rows-2">
            <StatCard
              title="一级代理人数"
              value={userStats.level1}
              icon={Briefcase}
            />
            <StatCard
              title="二级代理人数"
              value={userStats.level2}
              icon={Briefcase}
            />
            <StatCard
              title="三级代理人数"
              value={userStats.level3}
              icon={Briefcase}
            />
            <StatCard
              title="平台总资金结余"
              value={`$${platformTotalBalance.toFixed(2)}`}
              icon={Wallet}
              description="所有用户可用余额总和"
            />
          </div>
        </div>
      </div>
