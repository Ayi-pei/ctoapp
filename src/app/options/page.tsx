    ...
    <Table>
      ...
      <TableBody>
        {contracts.map((contract, index) => {
          ...
          return (
            <TableRow
              key={`${type}-${contract.strike_price}-${index}`}
              className={cn(
                contract.in_the_money &&
                  (type === "call" ? "bg-green-500/10" : "bg-red-500/10")
              )}
            >
              <TableCell>
                <div className="flex flex-col">
                  ...
                  >
                    ...
                    {contract.change_percent.toFixed(2)}%)
                  </span>
                </div>
              </TableCell>
...
const OptionsChainSkeleton = () => (
  <div className="grid md:grid-cols-2 gap-4">
    <Card>
      ...
      <CardContent>
        {[...Array(5)].map((_, i) => (
          <Skeleton key={`call-skeleton-${i}`} className="h-12 w-full my-2" />
        ))}
      </CardContent>
    </Card>
    <Card>
      ...
      <CardContent>
        {[...Array(5)].map((_, i) => (
          <Skeleton key={`put-skeleton-${i}`} className="h-12 w-full my-2" />
        ))}
      </CardContent>
    </Card>
  </div>
...
export default function OptionsPage() {
  ...
  useEffect(() => {
    if (optionsChain && optionsChain.length > 0) {
      const currentExpirationExists = optionsChain.some(
        (c) => c.expiration_date === selectedExpiration
      );
  ...
  const currentChain = optionsChain?.find(
    (c) => c.expiration_date === selectedExpiration
  );
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <Select
              ...
              disabled={!optionsChain || optionsChain.length === 0}
            >
              <SelectTrigger className="w-[180px]">
                <SelectValue placeholder="选择到期日" />
              </SelectTrigger>
              <SelectContent>{expirationOptions}</SelectContent>
            </Select>
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger>
                ...
                <TooltipContent>
                  <p className="max-w-xs">
                    高亮行表示该期权为价内期权
                    (In-the-Money)。数据每小时更新一次，其余时间为模拟数据。
                  </p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
