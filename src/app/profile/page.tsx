...
const ProfileHeader = () => {
    ...
    const getUsdtValue = (assetName: string, amount: number, summaryData: MarketSummary[]) => {
                    ...
                    <div className="space-y-1 md:space-y-2">
                        <div className="flex items-center gap-2">
                             {isEditingNickname ? (
                                ...
                                <div className="flex items-center gap-2">
                                     <h2 className="font-semibold text-xl md:text-2xl cursor-pointer text-foreground truncate" onClick={() => setIsEditingNickname(true)}>{nickname}</h2>
                                     <Edit2 className="w-4 h-4 text-muted-foreground cursor-pointer flex-shrink-0" onClick={() => setIsEditingNickname(true)} />
                                </div>
                            )}
                            {user?.is_admin && <Badge variant="destructive" className="text-xs"><Crown className="w-3 h-3 mr-1"/>Admin</Badge>}
                        </div>
                       
                        <p className="text-sm md:text-base font-semibold text-foreground/90">总资产估值: {totalBalance.toFixed(2)} USDT</p>
