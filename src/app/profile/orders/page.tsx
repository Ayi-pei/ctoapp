...
export default function ProfileOrdersPage() {
    ...
    const getStatusBadge = (order: OrderHistoryItem) => {
        if ('orderType' in order) { // It's a trade
        ...
        } else if ('type' in order && (order.type === 'deposit' || order.type === 'withdrawal')) { // It's a transaction request
    ...
    const getOrderType = (order: OrderHistoryItem) => {
         ...
         if ('type' in order) {
             const transaction = order as Transaction;
             return (
                 <span className={`px-2 py-1 text-xs font-semibold rounded-full ${transaction.type === 'deposit' ? 'bg-green-500/20 text-green-500' : 'bg-orange-500/20 text-orange-500'}`}>
    ...
    const getOrderAmount = (order: OrderHistoryItem) => {
        if ('orderType' in order) {
            return (order.orderType === 'spot' ? (order as SpotTrade).total : (order as ContractTrade).amount)?.toFixed(4)
        }
        if ('type' in order) {
                ...
                <Card>
                    ...
                    <CardContent>
                        <Table>
                            ...
                            <TableBody>
                                {allHistory.length > 0 ? allHistory.map((order) => (
                                    <TableRow key={order.id}>
                                        <TableCell>{'trading_pair' in order ? order.trading_pair : (order as Transaction).asset}</TableCell>
