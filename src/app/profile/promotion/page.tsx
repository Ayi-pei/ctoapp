...
export default function PromotionPage() {
  ...
  const calculateTeamStats = useCallback(
    (team: DownlineMember[], allRequests: AnyRequest[]) => {
  ...
  const totalCommission = teamRewardLogs.reduce(
  ...
  const stats = {
    ...
    todayEarnings: 0.0, // Placeholder
    yesterdayEarnings: 0.0, // Placeholder
      ...
      <div className="p-4 md:p-8 space-y-6">
        ...
        <Card className="bg-card/50">
          <CardContent className="p-4 grid grid-cols-4 gap-y-4">
            ...
            <StatCard
              label="今日有效人数"
              value={stats.todayEffectiveMembers}
            />
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2 p-2 pl-4 border rounded-md bg-muted">
                  ...
                  <Button
                    ...
                    size="icon"
                    className="h-8 w-8"
                    onClick={() => copyToClipboard(user?.invitation_code || "")}
                  >
                    <Copy className="h-4 w-4" />
                  </Button>
                </div>
